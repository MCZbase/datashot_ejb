 ...
 */
@Entity
@Table(name = "MCZBASE_AUTH_AGENT_NAME", catalog = "lepidoptera", schema = "")
@NamedQueries({
	@NamedQuery(name = "MCZbaseAuthAgentName.findAll", query = "SELECT g FROM MCZbaseAuthAgentName g")
})
public class MCZbaseAuthAgentName {
	
    @Id
    @Basic(optional = false)
    @Column(name = "AGENT_NAME_ID", nullable = false)
	private int agent_name_id; // AGENT_NAME_ID bigint
    @Column(name = "AGENT_ID")
    private int agent_id; // donor_card_present_fg int,
    @Column(name = "AGENT_NAME_TYPE", length = 18) 
    private String agent_name_type; // agent_name_type varchar(18 char) 
    @Column(name = "DONOR_CARD_PRESENT_FG")
    private int donor_card_present_fg; // donor_card_present_fg int
    @Column(name = "AGENT_NAME", length = 184) 
	 ...
	 */
	public int getDonor_card_present_fg() {
		return donor_card_present_fg;
	}
	/**
	 * @param donor_card_present_fg the donor_card_present_fg to set
	 */
	public void setDonor_card_present_fg(int donor_card_present_fg) {
